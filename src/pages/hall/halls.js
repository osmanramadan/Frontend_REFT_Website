...
function Halls() {
  const [halls, _loading, setTown, town, setSearch, search, onChangeCity] =
    GetHallsHook();
    ...
    <div style={{ backgroundColor: '#eef4fa' }}>
      ...
      >
        ...
        <Col xs="12" sm="6" md="5" lg="3">
          <SearchHallDropdownTowns
            searchid={cityid}
            town={town}
            onChange={searchlocation_in_halls}
            addhall={true}
          />
        </Col>
      </Row>
      {cities.length > 0 && currentItems.length > 0 ? (
        <Container className="mt-5">
          <Row
            className={`d-flex justify-content-center px-5 ${i18n.language === 'ar' ? 'rtl' : ''}`}
          >
            <Col
              xs="2"
              sm="2"
              md="2"
              lg="3"
              className="d-flex flex-column justify-content-start mb-2 align-items-end"
            >
              <p
                style={{
                  fontWeight: '600',
            ...
            {currentItems.length === 0 ? (
              <div
                className={`d-flex justify-content-center mb-2 fw-bold ${i18n.language === 'ar' ? 'rtl' : ''}`}
                style={{ fontSize: '30px' }}
              >
                {t('halls.noHalls')}
              </div>
