...
import { faCaretUp, faLocationPin } from '@fortawesome/free-solid-svg-icons';
import { Col, Row } from 'react-bootstrap';
import { useTranslation } from 'react-i18next';

function SearchHallDropdownTowns({ addhall, searchid, town, onChange }) {
  ...
  return (
    ...
    >
      <p className="dropdown-btn">
        ...
        >
          <Col
            className={i18n.language === 'en' ? 'text-start' : 'text-end'}
      ...
      >
        {data &&
          data[searchid ? searchid - 1 : 1].towns.length &&
          data[searchid ? searchid - 1 : 1].towns.map((town, index) => (
            <div
              className="item text-center"
              style={{ fontFamily: 'cairo' }}
              key={index}
              onClick={() => changeLocation(town)}
            >
...
export default SearchHallDropdownTowns;
