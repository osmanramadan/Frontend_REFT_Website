    ...
    >
      <p className="dropdown-btn">
        <Row className="d-flex justify-content-between"  style={{fontWeight:"400",color:"#8E8EA0",fontSize:"20px",fontFamily:"cairo","direction":i18n.language === 'en'?'ltr':'rtl'}}>
          ...
          <Col className={i18n.language==='en'?'text-start':'text-end'} style={{fontFamily:'cairo'}} >{town?i18n.language==='en'?town.en:town.ar:i18n.language === 'en'?'Town':'مدنيتك'}</Col>

          <Col className={i18n.language==='en'?'text-end mx-2':'text-start mx-2'} >

            <FontAwesomeIcon  style={{ color:'#A1A1A1' }} icon={faLocationPin} />{' '}
          </Col>
        </Row>
      </p>
      ...
      >
          ...
          {data && data[searchid?searchid-1:1].towns.length &&  data[searchid?searchid-1:1].towns.map((town, index) => (
          <div className="item text-center" style={{fontFamily:'cairo'}} key={index} onClick={()=>changeLocation(town)}>
